...
class PgConversationRepository {
  ...
  static async addBookingIntroMessage({
    ...
    if (existingMessage.rows[0]) {
      return ConversationMessage.fromDatabase(existingMessage.rows[0]);
    }
    ...
    return ConversationMessage.fromDatabase(messageResult.rows[0]);
  }
  ...
  static async listForUser(userId, { scope = "active" } = {}) {
    ...
    return Conversation.fromDatabaseList(
      result.rows.map((row) => ({
        ...row,
  ...
  static async getByIdForUser({ conversationId, userId, markAsRead = true }) {
    ...
    try {
      ...
      return Conversation.fromDatabase({
        ...conversation,
        unread_count: 0,
        ...
        messages: ConversationMessage.fromDatabaseList(messagesResult.rows),
      });
    } catch (error) {
...
export default PgConversationRepository;
